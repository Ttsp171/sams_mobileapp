...
class BuildingReport extends StatefulWidget {
  ...
  @override
  State<BuildingReport> createState() => _BuildingReportState();
}

class _BuildingReportState extends State<BuildingReport> {
  ...
  getCSVPdfDownloadLink(type) async {
    ...
    final res = await HttpServices().getWithToken(
        '/api/get-building-reports?type=$type${widget.customUrl == "" ? "" : widget.customUrl}',
        context);
    if (res["status"] == 200) {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        title: const Text('Building Reports',
