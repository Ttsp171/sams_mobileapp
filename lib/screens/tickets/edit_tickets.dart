...
class EditTickets extends StatefulWidget {
  ...
  const EditTickets({super.key, required this.ticketData});
...
class _EditTicketsState extends State<EditTickets> {
  ...
  setAllData() {
    setState(() {
      employeeName = widget.ticketData["employee_name"];
      contactNo = widget.ticketData["contact_no"];
    });
  }
  ...
  getAllData(context) async {
    ...
    await getDropDownData(
        '/api/get-fmticket-city?company_id=${widget.ticketData["Company_ID"].toString()}',
        "city",
        context);
    await getDropDownData(
        '/api/get-fmticket-buildings?city_id=${widget.ticketData["city_id"].toString()}',
        "building",
        context);
    await getDropDownData(
        '/api/get-fmticket-rooms?building_id=${widget.ticketData["building_no"].toString()}',
        "room",
        context);
  ...
  String getCompanyName() {
    String companyNameCustom = "";
    for (var company in companyDropDown) {
      if (company["Company_ID"].toString() ==
          widget.ticketData["Company_ID"].toString()) {
        setState(() {
          companyNameCustom = company["company_name"]??"";
        });
      }
    }
    return companyNameCustom;
  }
